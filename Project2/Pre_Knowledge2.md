...
---
## 先備知識 - BPSK 在 AWGN 通道下的錯誤機率推導
### 🔹 BPSK 在 AWGN 通道下的錯誤機率推導
...
* 傳送 \$+\sqrt{E\_b}\$ 時，錯誤條件是：
  $y = +\sqrt{E_b} + n < 0 \Rightarrow n < -\sqrt{E_b}$
* 傳送 \$-\sqrt{E\_b}\$ 時，錯誤條件是：
  $y = -\sqrt{E_b} + n > 0 \Rightarrow n > \sqrt{E_b}$

這兩種情況對稱（因為雜訊 \$n\$ 是零均值 Gaussian），所以只需要計算其中一種情況即可。

#### 3. 錯誤事件

以傳送 \$-\sqrt{E\_b}\$ 為例，錯誤條件：
$n > \sqrt{E_b}$
...
已知 \$n \sim \mathcal{N}(0, \sigma^2)\$，其中 \$\sigma^2 = N\_0/2\$。
定義標準常態變數：
...
* `Dkreceiver`：等化後、去掉 DC 與 guard band 的頻域資料子載波，大小 = (52 × Nsymbol)。
* `reshape(..., 1, ofdmbit*Nsymbol)`：把矩陣展平成一維向量，長度 = `52 * Nsymbol`。
* 得到接收端的「所有資料子載波的估計值」`Dkr`。

👉 對應傳送端最初的 bit 資料，完成並列 → 串列 (Parallel → Serial) 的轉換。

---

### 2.2. 硬判決 (Hard Decision)

```matlab
Dkre = sign(real(Dkr));
```

* `real(Dkr)`：取實部，因為 BPSK 星座圖點在實軸 (±1)。
* `sign(...)`：根據正負號判斷符號：
...
* `dk`：傳送端的原始 BPSK 符號 (±1)。
* `Dkre ~= dk`：找出接收判決與原始符號不同的地方。
* `find(...)`：回傳所有錯誤 bit 的索引。
* `length(find(...))`：計算錯誤 bits 的數量。
* 除以 `(52*Nsymbol)`：錯誤 bits / 總 bits = **BER**。
* 存進 `ber(Eb_N0/2+1)`：對應不同 \$E\_b/N\_0\$ 測試點的 BER。
...
1. 把接收符號展平成 bit 向量。
2. 依照 BPSK 判決規則 (取 sign)，還原出接收 bits。
3. 與傳送端 bits 比對，計算錯誤數量。
4. 除以總 bits 數，得到 BER。
5. 存入 `ber` 陣列，用於後續繪製 BER 曲線。
