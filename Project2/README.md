...
% 把 subcarrier 放入題目所給的 data subcarrier
% 注意在data subcarrier上32與34之間沒有第33號sub carrier
Dk = zeros(M, Nsymbol);
Dk([7:32],:) = dk_sym(1:26,:);             
Dk([34:59],:) = dk_sym(27:52,:);

%---------------------M-point IDFT (轉換到時域data)---------------------%
IDFT = [];
for i = 1:Nsymbol
    IDFT = [IDFT ifft(Dk(:,i),M)];
end
...
%---------------------receiving signal z[n] = x[n]*h[n] + n[n]----------%
h = [0.5+0.5j 0.15+0.12j 0 0 -0.1+0.05j];
% parallel to serial conversion
x_n = reshape(D_cp,1,(M+16)*Nsymbol);
h_n = h./norm(h);
xh = conv(x_n,h_n);
...
for Eb_NO_dB = 0:2:8
    % Theoretical BER
    BPSK_Pb(Eb_NO_dB/2+1) = 0.5*erfc(sqrt(10^(Eb_NO_dB/10)));
    N0 = Eb*10^(-(Eb_NO_dB/10));
    
    n = sqrt(64/52)*sqrt(N0/2)*(randn(1,length(D_cp(:))*Nsymbol) + ...
        1i*randn(1,length(D_cp(:))*Nsymbol));
    z_n = xh(1:end-5) + n;
    ...
    Dkr = reshape(Dkreceiver,1,ofdmbit*Nsymbol);
    Dkre = sign(real(Dkr));
    ber(Eb_NO_dB/2+1) = length(find(Dkre ~= dk))/(52*Nsymbol);
end

%---------------------畫圖---------------------------------------------%
semilogy(0:2:8,ber,'r-o'); hold on;
semilogy(0:2:8,BPSK_Pb,'b-*'); hold on; grid on;
title('Bit error probability curve for BPSK using OFDM');
